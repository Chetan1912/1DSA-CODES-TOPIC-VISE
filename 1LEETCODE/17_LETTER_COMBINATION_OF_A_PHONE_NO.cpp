...
{
private:
    void solve(string digit, string output, int index, vector<string> &ans, string mapi[])
    {
        // base case
        ...
        string value = mapi[number];
        for (int i = 0; i < value.length(); i++)
        {
            output.push_back(value[i]);
            solve(digit, output, index + 1, ans, mapi);
            output.pop_back();
        }
    }
...
public:
    vector<string> letterCombinations(string digits)
    {
        ...
        string output;
        int index = 0;
        solve(digits, output, index, ans, mapi);
        return ans;
    }
};